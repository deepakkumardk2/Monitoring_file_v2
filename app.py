...
#         if context == 'nfms':
#             if isinstance(row[0], (datetime, date)) and c3:
#                 records_loaded_raw = row[8] if len(row) > 8 else None
#                 meter_shared = extract_leading_number(records_loaded_raw)
#                 if c3 == 'BLOCK' and meter_shared is not None:
#                     records_shared = meter_shared * 48
#                 else:
#                     records_shared = as_int(records_loaded_raw)
#                 nfms[c3] = {
#                     'eligible': as_int(row[6]) if len(row) > 6 else None,
#                     'meter_shared': meter_shared if c3 == 'BLOCK' else (as_int(row[7]) if len(row) > 7 else None),
...
# def build_output_filename(ws, report_date, file_date=None, cluster_label=None):
#     cluster = secure_name(cluster_label or ws['C1'].value or 'Triveni', 'Triveni')
#     name_date = file_date or report_date
#     return f"Job Monitoring UPPCL_{name_date.strftime('%d%m%Y')}__{cluster}.xlsm"
...
# def parse_billing_excel(path, cluster):
#     target = (cluster or '').strip().upper()
#     wb = load_workbook(path, data_only=True)

#     sheet = None
#     for name in wb.sheetnames:
#         if name.strip().lower() == 'consolidate':
#             sheet = wb[name]
#             break
#     if sheet is None:
#         raise ValueError("Could not find 'Consolidate' sheet in Billing Excel.")
...
#         label = norm_text(sheet.cell(r, 1).value)
#         if label == 'CLUSTER':
#             cluster_row = r
#         elif label == 'CONSUMER COUNT':
#             consumer_row = r
#         elif label in ('START DATE TIME', 'START TIME', 'START DATE/TIME'):
#             start_row = r
#         elif label == 'DURATION':
#             duration_row = r

#     if not cluster_row:
#         raise ValueError("Cluster header row not found in Consolidate sheet.")

#     column_index = None
#     cluster_label = None
#     for c in range(2, sheet.max_column + 1):
#         header = norm_text(sheet.cell(cluster_row, c).value)
#         if header == target:
#             column_index = c
#             cluster_label = CLUSTER_LABELS.get(target, sheet.cell(cluster_row, c).value)
#             break
#     if not column_index:
#         raise ValueError(f"Cluster '{cluster}' not found in Consolidate sheet headers.")

#     consumer_count = as_int(sheet.cell(consumer_row, column_index).value) if consumer_row else None
#     start_value = sheet.cell(start_row, column_index).value if start_row else None
#     duration_value = sheet.cell(duration_row, column_index).value if duration_row else None

#     if consumer_count is None:
#         raise ValueError(f"Consumer Count missing for cluster {cluster} in Consolidate sheet.")

#     start_time_only = None
#     if isinstance(start_value, datetime):
#         start_time_only = start_value.time().replace(microsecond=0)
#     elif hasattr(start_value, 'hour') and hasattr(start_value, 'minute'):
#         start_time_only = start_value
#     elif isinstance(start_value, str):
#         try:
#             start_time_only = parse_time_value(start_value)
#         except Exception:
#             start_time_only = None

#     duration_text = None
#     if hasattr(duration_value, 'hour') and hasattr(duration_value, 'minute'):
#         duration_text = f"{duration_value.hour:02d}:{duration_value.minute:02d}:{getattr(duration_value, 'second', 0):02d}"
#     elif isinstance(duration_value, datetime):
#         duration_text = duration_value.strftime('%H:%M:%S')
#     elif isinstance(duration_value, str):
#         cleaned = duration_value.strip()
#         if re.fullmatch(r'\d{1,2}:\d{2}:\d{2}', cleaned):
#             duration_text = cleaned
#         else:
#             m = re.search(r'(\d{1,2}:\d{2})', cleaned)
#             if m:
#                 duration_text = m.group(1) + ':00'
#     elif is_number(duration_value):
#         total_seconds = int(round(float(duration_value) * 86400))
...
#     prepaid = extract_prepaid_summary_totals(prepaid_summary_path)
#     processed = parse_processed_log(processed_log_path)
#     billing_info = parse_billing_excel(billing_excel_path, cluster)

#     billing_count = billing_info['consumer_count']
#     time_of_run = billing_info['start_time']
#     daily_billing_duration = billing_info['duration_text']

#     report_date = processed['report_date']
#     if file_date is None:
#         file_date = datetime.today().date()

#     cluster_label = billing_info.get('cluster_label')
...
#     ws1[f'D{prepaid_value_row}'] = prepaid['converted_to_prepaid']
#     ws1[f'E{prepaid_value_row}'] = billing_count
#     if time_of_run is not None:
#         ws1[f'F{prepaid_value_row}'] = time_of_run
#     if daily_billing_duration is not None:
#         ws1[f'G{prepaid_value_row}'] = daily_billing_duration
...
#     ws1[f'F{prepaid_value_row}'].number_format = 'h:mm:ss AM/PM'
#     ws1[f'G{prepaid_value_row}'].number_format = '[hh]:mm:ss'
...
#         'sms_date': sms_date.strftime('%d/%m/%Y'),
#         'sms_total_count': total_count,
#         'cluster': billing_info.get('cluster_label'),
#         'consumer_count': billing_count,
#         'time_of_run': str(time_of_run) if time_of_run else None,
#         'daily_billing_duration': daily_billing_duration,
#         'normal_meter': normal_meter,
#         'net_meter': net_meter,
#         'cumulative_data_shared_for_month': cumulative_monthly_total,
#         'download_name': build_output_filename(ws1, report_date, file_date, billing_info.get('cluster_label')),
#     }
...
#             <div>
#                 <label>Billing Excel (Consolidate)</label>
#                 <input type="file" name="billing_excel_file" accept=".xlsx,.xlsm" required>
#             </div>

#             <div>
#                 <label>Cluster</label>
#                 <select name="cluster" required style="width:100%; box-sizing:border-box; padding:10px 12px; border:1px solid #ccd6e0; border-radius:8px; font-size:14px;">
#                     <option value="Triveni">Triveni</option>
...
#         prepaid_summary_file = request.files.get("prepaid_summary_file")
#         processed_log_file = request.files.get("processed_log_file")
#         billing_excel_file = request.files.get("billing_excel_file")
...
#         if not processed_log_file or not processed_log_file.filename:
#             return jsonify({"error": "Processed Data Log file is required."}), 400
#         if not billing_excel_file or not billing_excel_file.filename:
#             return jsonify({"error": "Billing Excel file is required."}), 400
...
#         if not cluster:
#             return jsonify({"error": "Cluster selection is required."}), 400
#         if not normal_meter_raw:
#             return jsonify({"error": "Normal meters is required."}), 400
#         if not net_meter_raw:
#             return jsonify({"error": "Net meter is required."}), 400
...
#             prepaid_path = os.path.join(tmpdir, secure_name(prepaid_summary_file.filename, "prepaid_summary.xlsx"))
#             processed_path = os.path.join(tmpdir, secure_name(processed_log_file.filename, "processed_log.xlsx"))
#             billing_excel_path = os.path.join(tmpdir, secure_name(billing_excel_file.filename, "billing_excel.xlsx"))

#             template_file.save(template_path)
#             prepaid_summary_file.save(prepaid_path)
#             processed_log_file.save(processed_path)
#             billing_excel_file.save(billing_excel_path)

#             output_stream, meta = fill_template(
#                 template_path=template_path,
#                 prepaid_summary_path=prepaid_path,
#                 processed_log_path=processed_path,
#                 billing_excel_path=billing_excel_path,
#                 cluster=cluster,
#                 sms_raw_text=sms_raw_text,
#                 normal_meter=normal_meter,
#                 net_meter=net_meter,
...
def fill_template(
    ...
    source_report_date = processed['report_date']
    display_date = source_report_date
    if file_date is None:
    ...
    sms_payload = parse_sms_raw_text(sms_raw_text)
    sms_date = sms_date_override or (display_date + timedelta(days=1))

    prepaid_header_row = find_row_by_col_text(ws1, 2, 'PREPAID BILLING DATE', exact=True)
    prepaid_value_row = prepaid_header_row + 1 if prepaid_header_row else 3
    ws1[f'B{prepaid_value_row}'] = display_date
    ws1[f'C{prepaid_value_row}'] = 'YES'
    ws1[f'D{prepaid_value_row}'] = prepaid['converted_to_prepaid']
    ws1[f'E{prepaid_value_row}'] = billing_count
    if time_of_run is not None:
    ...
    nfms_header_row = find_row_by_col_text(ws1, 2, 'Meter Data(RTC) DATE', exact=True) or 11
    processed_scan_end = max(processed_header_row + 1, nfms_header_row - 2)
    for row_no in range(processed_header_row + 1, processed_scan_end + 1):
        label = norm_text(ws1[f'C{row_no}'].value)
        if label in label_to_profile:
            processed_row_map[label_to_profile[label]] = row_no
    fallback_rows = [('DAILY', processed_header_row + 1), ('BILLING', processed_header_row + 2), ('BLOCK', processed_header_row + 3)]
    for profile, default_row in fallback_rows:
        processed_row_map.setdefault(profile, default_row)
    for profile, row_no in processed_row_map.items():
        values = regular.get(profile, {})
        ws1[f'B{row_no}'] = display_date
        ws1[f'D{row_no}'] = values.get('raw')
        ws1[f'E{row_no}'] = values.get('processed')
        ws1[f'F{row_no}'] = values.get('exception')
    ...
    monthly_row = find_or_create_sheet2_monthly_row(ws2, display_date)
    ws2.cell(monthly_row, 1).value = display_date
    ws2.cell(monthly_row, 2).value = normal_meter
    ws2.cell(monthly_row, 3).value = net_meter
    cumulative_monthly_total = compute_sheet2_monthly_total(ws2)
    ...
    monthly_header_row = find_row_by_col_text(ws1, 2, 'Monthly Billing DATE', exact=True) or 32
    monthly_value_row = monthly_header_row + 1
    ws1[f'B{monthly_value_row}'] = display_date
    ws1[f'C{monthly_value_row}'] = prepaid['total_consumers']
    ws1[f'D{monthly_value_row}'] = cumulative_monthly_total
    ...
    jio_header_row = find_row_by_col_text(ws1, 2, 'JIO Data Sharing:DATE', exact=True) or 32
    jio_value_row = jio_header_row + 1
    ws1[f'B{jio_value_row}'] = display_date
    ws1[f'C{jio_value_row}'] = jio.get('BLOCK')
    ws1[f'D{jio_value_row}'] = jio.get('DAILY')
    ws1[f'E{jio_value_row}'] = jio.get('BILLING')
    ...
    return output, {
        'latest_processed_sheet': processed['sheet_name'],
        'source_report_date': source_report_date.strftime('%d/%m/%Y'),
        ...
        'net_meter': net_meter,
        'cumulative_data_shared_for_month': cumulative_monthly_total,
        'download_name': build_output_filename(ws1, display_date, file_date, billing_info.get('cluster_label')),
    }
